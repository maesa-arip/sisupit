...
- **Sisa (OPEN):** (a) Tile server self-hosted penuh (TileServer-GL/OpenMapTiles) = proyek infra terpisah; kini tetap CARTO tapi sudah terpusat. (b) Kode app FINDINGS #34/#35 (GPS fix, sentralisasi tile, hardening default) belum di-push ke VPS — tak wajib karena env var di-set eksplisit & controller sudah baca config. (c) Data VPS baru Bali; Indonesia penuh = swap PBF + reimport.
