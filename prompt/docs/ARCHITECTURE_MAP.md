...
| Berita Acara | Laporan Kegiatan Penyelamatan yang diisi petugas pasca-insiden (FINDINGS #39). Append-only: entri `sementara` (data awal) & `final` (hasil investigasi) disimpan terpisah. Korban banyak; foto KTP di disk privat + route bergerbang. **Sumber Informasi terisi sendiri hanya untuk laporan yang masuk lewat aplikasi** (TASK_45): pembedanya PERAN pemilik `reports.user_id`, karena `ReportController::store()` selalu menulis `auth()->id()` — laporan yang diketik operator ber-user_id operator itu, dan kolomnya sengaja dibiarkan kosong. Kalimatnya hidup di `ReportResolution::SUMBER_APLIKASI`, jangan ditulis ulang di tempat lain. **OPD yang dilibatkan ikut "Tim yang atensi di TKP"** bertanda `(OPD)`, dibaca dari kolom snapshot `report_agencies.agency_name` (bukan master `agencies`) agar berita acara lama tak berubah saat master di-rename. `resolve()` TIDAK diubah — berita acara diisi belakangan | `app/Http/Controllers/ReportResolutionController.php`, `resources/js/Pages/Front/Reports/Resolution/Create.jsx` |
