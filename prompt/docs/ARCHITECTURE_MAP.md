...
app/
  ...
  Events/        ResponderLocationUpdated, IncidentLocationCorrected, ReportStatusChanged,
                 ReportFeedChanged, ResponderRosterChanged,
                 ReportRecordChanged (#113 - catatan insiden berubah tanpa status/roster
                 ikut berpindah: OPD dilibatkan/dilepas/mengonfirmasi, entri berita acara
                 dibuat/dihapus, pelapor menyunting laporannya) - semua broadcast via Reverb
...
docker/tiles/     Self-hosted basemap (TileServer-GL + tilemaker, Bali) — tile ditarik BROWSER,
                  jadi WAJIB publik (Nginx /tiles/), beda dari dua di atas (TASK_46, #92)
```
...
|-------|----------------|-----------|
| Laporan Darurat | CRUD laporan milik sendiri (status TERLAPOR→pending→handling→resolved/ditolak). Edit (TASK_16/#30) = pelapor saja & hanya saat TERLAPOR: konten + kelola galeri foto, lokasi tak diubah. **Penetapan lokasi punya SATU mode sejak 2026-09-01** (permintaan user; mencabut dua mode TASK_28). Titik pin adalah sumber kebenaran wilayah untuk SEMUA pelapor: geser pin / klik peta / pilih hasil pencarian → provinsi..desa ditulis ulang dari reverse-geocode titik itu; memilih dropdown wilayah → pin melompat ke centroid `meta` wilayah itu (alur telepon Pusat Komando tetap terlayani). Sinkron dua arah, tanpa sakelar. Sakelar lama "Pilih manual / Ikuti pin peta" dibuang karena mode `manual` MEMBLOKIR penulisan kode wilayah dari pin, sehingga pin & dropdown bisa menunjuk dua tempat berbeda tanpa satu pun tanda di layar. Blok "Wilayah Kejadian" (kotak cari + empat dropdown) & panel "Alamat Lengkap (otomatis)" kini tampil untuk **semua** pelapor: `ReportRequest` mewajibkan `village_code` untuk SETIAP laporan baru tanpa membedakan peran, sementara pencocokan nama OSM kerap berhenti di kecamatan — selama blok itu disembunyikan dari warga, warga yang mengalaminya ditolak server pada field yang tak pernah dirender. Prop `region_picker` dari `create()` **bukan lagi gerbang tampil**, hanya NILAI AWAL kode wilayah (yurisdiksi operator; `null` untuk non-staf) — jangan dijadikan gerbang lagi. Dijaga `ReportLocationSingleModeTest` | `app/Http/Controllers/ReportController.php`, `resources/js/Pages/Front/Reports/Create.jsx` |
| Workflow Respons | approve/reject/takeAction/cancelResponse/arrive/resolve/updateLocation/correctLocation. `reject` (TASK_10) = status `ditolak` (arsip) — punya chip filter sendiri di `/admin/reports` & Peta Pemantauan sejak TASK_48/#94, tapi **tidak** ditampilkan ke pemantau (pejabat/relawan) karena `ReportController::index` memang tak mengirimkannya. **Penutupan & penolakan meninggalkan jejak PELAKU** (TASK_44, #88): `resolve()` mengisi `reports.resolved_by`+`resolved_at`, `reject()` mengisi `rejected_by` di samping `rejected_at`/`rejected_reason` yang sudah ada. `resolved_at` BUKAN kembaran "Jam Selesai" di rekap — yang itu diturunkan dari `finished_at` responder terakhir. `cancelResponse` (TASK_13) = batal meluncur saat `en_route` (revert handling→pending bila responder terakhir mundur). take-action/arrive ter-scope wilayah laporan (TASK_12). **VERIFIKASI = ADMIN SAJA sejak TASK_51/#101** (keputusan user 2026-08-31): `approve()`, `reject()`, dan `removeAgency()` menolak `petugas`. Petugas tetap memegang seluruh aksi lapangan (takeAction/arrive/cancelResponse/resolve/correctLocation), tetap menerima notifikasi laporan masuk, dan tetap boleh MEMINTA bantuan OPD lewat `notifyAgencies()` — asimetri yang disengaja: eskalasi lahir di lapangan, pencabutan koordinasi tidak. Panel tombolnya di layar dibaca dari prop server `canVerify`/`canRemoveAgencies` (`ReportController::show`), **bukan** dari daftar peran di JSX; kalau daftar peran di controller ini diubah, ubah juga `$isVerifier` di sana atau tombol dan gerbang berhenti menjawab pertanyaan yang sama (bentuk #94) | `app/Http/Controllers/ReportActionController.php` |
...
| Berita Acara | Laporan Kegiatan Penyelamatan yang diisi petugas pasca-insiden (FINDINGS #39). Append-only: entri `sementara` (data awal) & `final` (hasil investigasi) disimpan terpisah. Korban banyak; foto KTP di disk privat + route bergerbang. **Sumber Informasi terisi sendiri hanya untuk laporan yang masuk lewat aplikasi** (TASK_45): pembedanya PERAN pemilik `reports.user_id`, karena `ReportController::store()` selalu menulis `auth()->id()` — laporan yang diketik operator ber-user_id operator itu, dan kolomnya sengaja dibiarkan kosong. Kalimatnya hidup di `ReportResolution::SUMBER_APLIKASI`, jangan ditulis ulang di tempat lain. **OPD yang dilibatkan ikut "Tim yang atensi di TKP"** bertanda `(OPD)`, dibaca dari kolom snapshot `report_agencies.agency_name` (bukan master `agencies`) agar berita acara lama tak berubah saat master di-rename. `resolve()` TIDAK diubah — berita acara diisi belakangan. **TASK_49:** kolom `volume_air` (volume air yang dipakai; TEKS BEBAS seperti `kerugian`, sebab yang ditulis di lapangan "±3 tangki") & `report_victims.kondisi` (Kondisi Korban — ikut dihitung sebagai isi baris, supaya korban yang baru diketahui kondisinya tak dilewati sebagai baris kosong). **Entri `final` hanya boleh ditutup admin/superadmin** (`canFinalize()`, dicek di server — tombol yang disembunyikan tidak menghentikan request buatan tangan); petugas tetap mengisi `sementara`. Ikutannya: antrian "Menunggu Berita Acara" di dashboard petugas menyaring **belum ada entri SAMA SEKALI**, bukan lagi "belum final" — kalau tidak, insiden yang sudah ia isi menggantung selamanya menunggu admin, dan antrian yang tak bisa dibereskan sendiri terbaca sebagai bug | `app/Http/Controllers/ReportResolutionController.php`, `resources/js/Pages/Front/Reports/Resolution/Create.jsx` |
| Tracking Lokasi | Riwayat append-only + broadcast WebSocket ke command center. Status laporan juga disiarkan real-time via `ReportStatusChanged` (TASK_14) di channel privat yang sama → halaman Show update tanpa refresh. **Sejak #113 channel itu memikul LIMA event**, dan aturannya satu: **tiap tulisan ke catatan sebuah insiden wajib punya aba-abanya** — selama tiga kelompok mutasi (pelibatan & konfirmasi OPD, entri berita acara, suntingan pelapor) tidak menyiarkan apa pun, halaman detail menampilkan keadaan yang sudah tidak berlaku tanpa satu pun tanda bahwa ia basi. Yang disiarkan **aba-aba, bukan data** (`ReportRecordChanged` hanya membawa `reportId`), sebab channel ini juga didengar pelapor & relawan; yang menyajikan isinya tetap server lewat `router.reload()` sehingga gerbang per-peran di `ReportController::show()` dihitung ulang untuk tiap penonton. Di sisi klien KETIGA sinyal (`ReportStatusChanged`, `ResponderRosterChanged`, `ReportRecordChanged`) memanggil SATU `reloadIncident()` dengan SATU daftar prop `['report','reportAgencies','resolutions']` — `reportAgencies` & `resolutions` adalah prop terpisah yang bentuk lama `only: ['report']` tak pernah sentuh, dan daftar yang berbeda-beda membuat sinyal yang datang belakangan membatalkan permintaan yang lebih lengkap (Inertia hanya menerbangkan satu kunjungan pada satu waktu) | `app/Models/TrackingLog.php`, `app/Events/ResponderLocationUpdated.php`, `app/Events/ReportStatusChanged.php`, `app/Events/ReportRecordChanged.php` |
...
| Halaman Informasi & Legal | Lima halaman publik tanpa auth (TASK_19): `/syarat-ketentuan`, `/kebijakan-privasi`, `/pusat-bantuan`, `/tentang`, `/paket-lisensi`. Isi dokumen statis di komponen React (pola `/guideline`), tetapi instansi penyelenggara diambil dari tenant hasil subdomain & klausul lisensi dari `tenants.edition` (sewa/beli) — kabupaten baru cukup diisi lewat `/admin/tenants`. Identitas penyedia sistem, versi & tanggal berlaku dokumen, retensi ada di `config/legal.php` (override lewat `.env`). Persetujuan S&K saat daftar dicatat di `users.terms_accepted_at`. **2026-08-28:** entri menunya dihapus dari `navItems.js` atas permintaan user — kelima halaman tetap hidup & bisa diakses langsung lewat URL, tapi satu-satunya tautan yang tersisa di aplikasi ada di footer `AppLayout`. **2026-08-07:** penyedia = `PT Tawarin Dimana Saja`; `/syarat-ketentuan` memuat DUA dokumen dalam satu halaman bertab — *Pengguna Umum* (warga/petugas/relawan) & *Pengguna Berkontrak* (instansi ber-PKS: wanprestasi, non-refundable, pengesampingan Ps. 1266 KUHPerdata, batas ganti rugi). Larangan resell/white-label berlaku penuh termasuk paket `beli`. Kontak legal terpisah dari dukungan teknis (`penyedia.email_legal`); `penyedia.alamat` menentukan yurisdiksi PN & baris disembunyikan bila kosong | `app/Http/Controllers/InfoController.php`, `resources/js/Pages/Info/*`, `config/legal.php` |
| Auth & Identitas | Breeze + Socialite (Google), auto-assign role `warga` (bernama `masyarakat` sampai 2026-09-02; diganti migrasi `2026_09_02_100000`, bukan seeder), validasi profil | `app/Http/Controllers/Auth/SocialiteController.php`, `app/Http/Controllers/ProfileController.php` |
| Relawan | Toggle siaga, radar insiden di area relawan. **Pendaftaran mandiri relawan TIDAK LAGI PUNYA PINTU DI LAYAR sejak 2026-09-02** (permintaan user): kartu "Bantu Sesama" di `Pages/Dashboard.jsx` dan banner "Panggilan Kemanusiaan" di `Pages/Profile/Edit.jsx` — dua-duanya berujung tombol "Daftar Relawan" — dicabut. Peran `relawan` kini hanya diberikan admin lewat `/admin/users`. Route & controllernya SENGAJA dibiarkan hidup (`volunteer.register`, `admin.relawan.update`) tapi kini **tanpa satu pun pemanggil di frontend**; keduanya dulu dipanggil oleh dua tombol yang berbeda untuk tindakan yang sama, dan yang kedua itu justru endpoint tanpa authorize dari FINDINGS #1 (P0) — menghidupkan lagi ajakan itu berarti memilih salah satunya lebih dulu, jangan menyalin yang lama. Daftar relawan (`Front/RelawanController`) BUKAN publik: di-gate `role:petugas\|admin\|superadmin` + ter-scope yurisdiksi via `User::scopeIsAdmin()` (FINDINGS #22) | `app/Http/Controllers/VolunteerController.php`, `app/Http/Controllers/Front/RelawanController.php` |
| Pengumuman | Broadcast info publik | `app/Http/Controllers/Admin/AnnouncementController.php` |
| Forum Warga | Tanya jawab kesiapsiagaan warga, **satu ruang per kabupaten/kota** (TASK_54, 2026-09-14). Keputusan user: komunitas warga, pertanyaan baru `menunggu` sampai admin menyetujui (pra-moderasi), **lencana "Jawaban Resmi Damkar" hanya untuk admin/superadmin** (petugas membalas sebagai pengguna biasa), tanpa foto, **wajib login**. Isolasi wilayah seluruhnya dari `Tenantable` pada `ForumThread`, yang SENGAJA hanya menyimpan `province_code`+`city_code` dari AKUN penulis (district/village NULL = "berlaku sekabupaten" menurut Tenantable hierarkis #60) - menyimpan desa penulis akan menyembunyikan pertanyaannya dari desa lain di kabupaten yang sama tanpa galat. `ForumPost` & `ForumFlag` tanpa Tenantable dan SELALU dicapai lewat thread yang tersaring (tiap aksi atas `{post}` memeriksa thread induknya). Menyala per kabupaten lewat `tenants.features` (`Tenant::FEATURE_FORUM`, daftar putih `Tenant::FEATURES`, checkbox di `/admin/tenants`), dibaca dari `city_code` AKUN - bukan subdomain; mati = route 404 & menu absen (`auth.user.forum_enabled`, dihitung `ForumThread::enabledFor()` yang juga menggerbangi route). Moderator menyembunyikan dengan alasan (berjejak `moderated_by/at/reason`, bisa dipulihkan), tidak menghapus. **Notifikasinya `database` saja** (`ForumNotification`) - lihat CONVENTIONS | `app/Http/Controllers/Front/ForumController.php`, `app/Http/Controllers/Admin/ForumModerationController.php`, `app/Models/Forum{Thread,Post,Flag}.php`, `resources/js/Pages/Forum/*`, `resources/js/Pages/Admin/Forum/Index.jsx` |
...
| Basemap tiles | URL tile Leaflet terpusat di `MAP_TILE_URL` (`resources/js/lib/utils.js`) — dipakai 14 peta. Nilai di-inject RUNTIME dari `config('services.map.tile_url')` → `window.MAP_TILE_URL` (`app.blade.php`), swappable lewat 1 env var `MAP_TILE_URL` TANPA rebuild & TANPA deploy kode (pola sama Nominatim/OSRM). **Sejak TASK_46 (2026-08-27) basemap DI-SELF-HOST**: `docker/tiles/` menjalankan TileServer-GL atas vector tiles hasil `tilemaker` dari `bali.osm.pbf` milik Nominatim (style **Sisupit Light** + font Noto Sans). Style-nya IKUT REPO di `docker/tiles/style/sisupit-light/` (bind mount, sepola `config.json`) — turunan Positron v1.9 yang disetel 2026-08-28: air & taman dikembalikan berwarna, hierarki jalan dipertegas, seluruh label dipangkas ke `{name:latin}` (aksara asing pada nama POI ikut TERGAMBAR DI DALAM TILE — bentuk lain #83), dan fontstack dikunci ke Noto Sans alih-alih menumpang fallback Metropolis. Style OSM Bright yang dipakai 27 Agustus tidak lagi diunduh skrip mana pun. Mengganti style = sunting 1 JSON + restart container: TANPA membangun ulang mbtiles, TANPA rebuild, TANPA deploy; **id style tetap `sisupit`** karena id itu ada di dalam `MAP_TILE_URL` ketiga environment. Sebelumnya cadangannya menunjuk CARTO Voyager, dan saat CARTO mewajibkan API key SELURUH peta di prod/staging/dev tercoret "API KEY REQUIRED" sekaligus tanpa satu pun galat (FINDINGS #92) — karena itu **`MAP_TILE_URL` wajib diisi di tiap environment**; cadangan di config kini tile OSM resmi, jaring pengaman saja, bukan sumber produksi. Tile ditarik BROWSER (bukan server) → beda dari Nominatim/OSRM yang cukup loopback, tile server WAJIB publik; di VPS disajikan Nginx di `/tiles/` dengan `proxy_cache` (render raster memakai CPU: ~0,7 dtk saat dingin, ~0,1 dtk sesudahnya). Data vektor sampai z14, zoom lebih jauh dioverzoom dari z14 dan tetap tajam (diuji s/d z19). **Tiga kegagalan basemap yang semuanya menjawab HTTP 200 tanpa satu baris log:** tanpa `data/fonts/` tile tergambar TANPA nama jalan/desa ("peta polos" = periksa font, bukan style); tanpa `data/coastline/water_polygons.shp` laut sewarna daratan sehingga **pulau tak berbentuk** di zoom jauh; dan bila shapefile itu versi **3857** alih-alih **4326**, poligon lautnya membesar sebesar dunia dan **menutupi daratan**. Karena itu verifikasi basemap WAJIB melihat gambarnya di zoom kota DAN zoom pulau, bukan cuma status HTTP. Bbox tilemaker (113.80,-9.40,116.30,-7.60) sengaja LEBIH LEBAR daripada `extract-bali.ps1` — margin laut supaya tepi kotaknya tak terlihat; cakupan DATA tetap Bali. Semua 14 pemanggilan `L.tileLayer` WAJIB memasang `attribution` OpenStreetMap (data ODbL) | `resources/js/lib/utils.js`, `config/services.php`, `docker/tiles/`, `app/Http/Middleware`→`app.blade.php` |
...
| Push Notification | FCM untuk insiden; WebPush dimatikan & PWA web dihapus. Payload `EmergencyAlertNotification::toFcm()` **data-only** + blok `android` (priority high) **dan** blok `apns` (aps.alert/sound `sirine.caf`/interruption-level `time-sensitive`, header push-type `alert`) — iOS memperlakukan pesan tanpa `aps.alert` sebagai background push yang tak menampilkan UI, jadi kedua blok wajib ada berdampingan (TASK_26). `device_type` dikirim frontend (`ios`/`android`). Lifecycle token per-DEVICE: login memindahkan token ke user aktif (`FcmController::store`), logout melepas token device ini (`AuthenticatedSessionController::destroy` menerima `fcm_token` di body) agar HP berhenti dapat sirine setelah keluar. **Permintaan bantuan ke OPD** via `AgencyDispatchNotification` (FCM+database, TASK_27) — DITUJUKAN ke akun peran `opd` milik instansi yang dipilih, bukan siaran wilayah; blok `apns` ikut disertakan (aturan TASK_26) tapi tanpa sirine karena ini koordinasi mitra, bukan panggilan meluncur. **Kabar balik DARI OPD** via `AgencyConfirmationNotification` (FCM+database, TASK_30/#63) saat tindakan berkondisi dipenuhi (mis. PLN "listrik sudah dipadamkan") — penerimanya (diperluas TASK_49 atas permintaan user) Pusat Komando yang menaungi laporan (`notifiableForReport` dengan ceiling `KEY_NOTIFY_LEVEL_PETUGAS`), **relawan siaga** dengan ceiling-nya SENDIRI `KEY_NOTIFY_LEVEL_RELAWAN` + `is_standby` (persis aturan `approve()` — menyalin ceiling petugas ke sini diam-diam melebarkan jangkauan relawan di luar setelan admin), responder yang sedang di lokasi dari **KEDUA** tabel `report_officers` **dan** `report_helpers` (keanggotaan bisa dari luar wilayah siaran; tabel helper dulu tak pernah dibaca sehingga relawan yang sudah tiba di TKP justru satu-satunya yang tak tahu listrik sudah padam), dan **PELAPOR** yang menunggu di TKP; yang mencatat konfirmasi tidak dikabari tindakannya sendiri. Tanpa sirine — koordinasi, bukan panggilan meluncur. **SUARA BERTINGKAT (TASK_50):** `EmergencyAlertNotification` kini memikul DUA tahap lewat kunci `alert_stage` di payload FCM **dan** `toArray()` — `report_incoming` (laporan warga baru masuk, hanya ke Pusat Komando; **tidak** bersirine dan **tidak** lagi berkata "DARURAT KEBAKARAN!", sebab laporannya belum diverifikasi siapa pun) vs `dispatch` (siaran sesudah verifikasi; **satu-satunya** yang bersirine). Di **Android** suara tidak bisa ditentukan payload — ia melekat pada notification channel yang setelannya PERMANEN, jadi server hanya mengirim penanda dan wrapper memilih channel; payload tak dikenal jatuh ke channel darurat (keputusan user: gagal berisik lebih aman daripada gagal diam). Di **iOS** sebaliknya, berkasnya ditentukan `aps.sound` sehingga pembedaan selesai di server. **JANGAN menamai penanda tahap `type`**: `BroadcastNotificationCreated::broadcastWith()` melakukan `array_merge($data, ['type' => nama kelas])`, jadi kunci itu ditimpa di jalur siaran — Android akan melihat nilai kita sementara aplikasi desktop tidak, tanpa galat di mana pun (dijaga `NotificationSoundStageTest`). **Aplikasi desktop (.exe) tidak memakai FCM sama sekali** — ia mendengar Reverb di channel privat `App.Models.User.{id}`, sehingga notifikasi hanya sampai ke sana bila `via()`-nya memuat `'broadcast'`; karena itu kedua kelas Agency\* ikut disiarkan sejak TASK_50 (sebelumnya konfirmasi PLN tak pernah tiba di layar Pusat Komando). **Notif balik ke PELAPOR** tiap transisi status via `ReportStatusUpdatedNotification` (FCM+database, TASK_06) — sengaja TANPA nada khusus, memakai bunyi bawaan sistem (keputusan user 2026-08-28: satu-satunya notifikasi yang penerimanya warga biasa). **Lonceng web** (TASK_11): `notifications` di-share via `HandleInertiaRequests`, ditandai-baca lewat `NotificationController` (`notifications.read`/`readAll`), ditampilkan di header `AppLayout` — dan sejak 2026-09-06 (#46 FIXED) `AppLayout` berlangganan `Echo.private('App.Models.User.{id}').notification()` lalu memuat ulang kedua prop itu, jadi angkanya tak lagi menunggu pindah halaman. **Batasnya:** hanya notifikasi yang `via()`-nya memuat `'broadcast'` yang membangunkannya; `ReportStatusUpdatedNotification` (notif balik ke PELAPOR) sengaja belum ikut — payloadnya tanpa `alert_stage`, dan aturan TASK_50 "payload tak dikenal TETAP sirine" akan membuat .exe operator bersirine pada tiap transisi laporan yang ia ketik sendiri (#114) | `app/Notifications/EmergencyAlertNotification.php`, `app/Notifications/ReportStatusUpdatedNotification.php`, `app/Http/Controllers/NotificationController.php` |
...
- **Login**: Breeze (email/password) + Socialite Google (`Auth\SocialiteController`), auto-assign role `warga` untuk user baru via Socialite. **Peran ini bernama `masyarakat` sampai 2026-09-02** (permintaan user); namanya diganti migrasi `2026_09_02_100000_rename_role_masyarakat_to_warga` yang MENYUNTING baris yang sama (id tetap, jadi seluruh `model_has_roles` utuh) dan lebih dulu membuang peran hantu `warga` sisa #110 supaya tak menabrak `UNIQUE(name, guard_name)`. Nama peran itu hidup sebagai STRING di `assignRole()`/`hasRole()`/`User::role()`/kamus label - satu tertinggal = `RoleDoesNotExist` saat warga baru mendaftar; dijaga `RoleSourceSingleListTest`.
...
- **Selain staf** (warga/relawan) → profil belum lengkap, **bukan** yurisdiksi nasional.
...
- **Akun OPD tak punya riwayat berbasis `user_id` maupun wilayah** (TASK_45, #91). `ReportController::index()` punya cabang `agencyIndex()` sendiri untuknya: daftar insiden yang instansinya diminta membantu, `withoutGlobalScopes()` + re-check `agency_id`, dengan prop `scope: 'agency'` yang menyembunyikan kedua tab. Akun OPD tanpa instansi melihat KOSONG, bukan semuanya. Peran `opd` juga sudah bisa diberikan **admin wilayah** lewat `/admin/users` (#89) — bukan eskalasi, sebab `opd` di luar STAFF_ROLES dan pemilihan instansinya dijaga `Agency::whereKey()` ber-Tenantable.
...
| Report | hasMany ReportHelper (`helpers()`), ReportOfficer (`officers()`), **ReportPhoto (`photos()`)**, **ReportAgency (`reportAgencies()`)**; belongsTo User + wilayah | Pakai `Tenantable` + `SoftDeletes`; status string (TERLAPOR/pending/handling/resolved/**ditolak**), bukan enum (lihat CONVENTIONS). `ditolak` (TASK_10) + kolom `rejected_reason`/`rejected_at`. Jejak pelaku penutupan (TASK_44): `resolved_by`/`resolved_at`/`rejected_by`, nullable tanpa backfill. **Relasinya bernama `resolver()`/`rejector()`, BUKAN `resolvedBy()`/`rejectedBy()`** — model ini dikirim utuh ke halaman detail dan relasi diserialisasi ter-snake_case, jadi nama itu akan menimpa kolom `resolved_by` di JSON (pola yang diikuti: `ReportResolution::creator()`). **DUA kolom alamat, dua penulis** (TASK_49, #95): `address` = patokan yang DIKETIK warga (opsional untuk kebakaran — darurat-first), `geo_address` = alamat hasil reverse-geocode dari pin. Dulu satu kolom: `correctLocation()` menimpa patokan dengan `display_name` Nominatim, sehingga panel "Alamat Presisi" tak dijamin siapa pun. **Jangan menyalin yang satu ke yang lain.** Saat sebuah laporan diringkas jadi satu baris "di mana", pakai `Report::alamatTampil()` (kembaran klien: `alamatLaporan()` di `lib/utils.js`) — bukan `address` langsung, sebab kolom itu sah kosong. Galeri foto via `report_photos` (TASK_07); kolom `photo` = sampul (foto pertama). `incident_type` (TASK_27) menyimpan jenis kejadian dari form Lapor — sebelumnya dibuang setelah validasi; nilainya dari `Report::INCIDENT_TYPES` (sumber tunggal, dipakai `ReportRequest` & aturan auto-centang OPD). Nullable tanpa backfill: laporan lama tak menghasilkan rekomendasi OPD. Sejak TASK_47 form dipecah dua tab (Kebakaran / Non Kebakaran) dan daftarnya bertambah `kebakaran_lainnya` = **kebakaran** yang jenisnya diketik sendiri warga, jadi aturan darurat-first berlaku untuknya (foto/detail opsional); **hanya `lainnya` yang berarti darurat NON-kebakaran** — pembanding di `ReportRequest` sengaja tetap satu nilai itu, bukan "bukan jenis kebakaran", karena kolomnya nullable dan kosong tak boleh mendadak berarti wajib foto. Konstanta `Report::FIRE_INCIDENT_TYPES` menampung jenis kebakarannya (dibaca `AgencySeeder`)  **ASAL-USUL TITIK** (TASK_52, #104): `location_source` (`Report::LOCATION_SOURCES` = `gps_pelapor`/`ditandai_manual`/`tanpa_referensi`/`dikoreksi_petugas`) + `location_accuracy_m` + `reporter_distance_m`, nullable tanpa backfill. Diisi SERVER di `store()` dari jarak posisi pelapor ke pin (ambang `Report::JARAK_PELAPOR_MAKS_M` = 300 m, haversine di PHP karena SQLite tak punya `acos()`); koordinat pelapornya sendiri **tidak disimpan**, hanya jaraknya. Tidak ada nilai khusus untuk alur telepon Pusat Komando — menurunkannya dari PERAN akan mengklaim "titik dipilih operator" pada petugas yang benar-benar melapor dari TKP, dan laporan telepon memang jatuh dengan benar ke `ditandai_manual`. `correctLocation()` menimpanya jadi `dikoreksi_petugas` + mengosongkan jarak & akurasi, dan `IncidentLocationCorrected` ikut membawanya agar lencana di layar berubah tanpa muat ulang. Ketiganya nullable & seluruh field requestnya opsional supaya tak ada satu pun bagian fitur ini yang bisa menggagalkan laporan darurat (klien lama tak mengirimnya → "tidak tercatat") |
...
auth+verified           : /forum, /forum/tanya [GET/POST throttle:forum-thread], /forum/{thread}
                           [GET/DELETE], /forum/{thread}/balas & /lapor [throttle:forum-reply],
                           /forum/{thread}/balasan/{post}/{terbaik,lapor} + DELETE (TASK_54 -
                           gerbang fitur per kabupaten di controller, 404 bila mati)
role:admin|superadmin   : /admin/forum (antrean moderasi) + threads/{thread}/{approve,hide,restore,pin},
                           posts/{post}/{hide,restore}, flags/{type}/{id}/dismiss
```
...
3. **`DashboardController::index` "Semua Laporan" feed** mem-bypass `Tenantable` total dan mengirim
   kolom mentah (termasuk `phone`, `name`, alamat presisi) ke SEMUA user login (warga/relawan)
