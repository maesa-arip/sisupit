...
| Hydrant, Pompa, PosPemadam | relasi wilayah (province/city/district/village); Pompa & PosPemadam pakai SoftDeletes | Ketiganya kini pakai `Tenantable`+kolom wilayah. Pompa/PosPemadam: kolom GPS aslinya `lat`/`lng` (BUKAN `location_lat/lng` — `$fillable` lama salah, sudah diperbaiki ke `$guarded=[]`, FINDINGS #23). **Hydrant (TASK_30)** dapat `water_pressure` (`Keras`/`Sedang`/`Kecil`, nullable = belum disurvei) dan `debit_lpm` (liter/menit, opsional — angkanya dipegang PDAM). Satuan `debit_lpm` sengaja sama dengan `pompas.capacity_lpm` & `hydrant_wargas.debit_lpm` supaya rekap debit per desa (`Admin\PompaController::debitSummary`) bisa menjumlahkan semuanya tanpa konversi; rekap itu juga mengirim `unknown_debit` agar total tak dibaca sebagai angka pasti padahal ia batas bawah |
| HydrantWarga | relasi wilayah; `Tenantable`, `$guarded=[]` | Hydrant swadaya warga (TASK_30, tabel `hydrant_wargas`). **Skemanya sengaja kembar dengan `hydrants`** — pengecualian aturan yang disetujui user, lihat `prompt/docs/PENGECUALIAN_ATURAN.md` #1; kalau menambah kolom di salah satu model, cek yang satunya. Beda perilaku dari kembarannya cuma satu: `debit_lpm` **WAJIB** di `HydrantWargaController` (rekap debit desa berdiri di atasnya), sedangkan di hydrant resmi opsional. `toSkklRow()` membuatnya bisa berdampingan dengan `Pompa` di daftar/peta SKKL; `source` = `hydrant_warga` |
