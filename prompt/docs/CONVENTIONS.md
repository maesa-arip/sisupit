...
| Status/enum | **Campuran**: `Report::status` adalah string mentah (`'TERLAPOR'`, `'pending'`, `'handling'`, `'resolved'`) — TIDAK pakai PHP enum, padahal enum lain (`MessageType`, `TenantLevel`, `UserGender`) sudah ada dan dipakai konsisten | `app/Enums/TenantLevel.php` (enum yang benar) vs `app/Http/Controllers/ReportActionController.php:29` (string mentah) |
