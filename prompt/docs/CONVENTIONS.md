...
| Riwayat suntingan aset | **Siapa mengubah apa ditulis EKSPLISIT dari controller, dalam transaksi yang sama dengan simpannya** (TASK_59, `hydrant_logs`). Bukan model event: jejaknya menjawab "siapa lewat aplikasi", dan event ikut mencatat seeder/perintah artisan sebagai penyunting tanpa nama. Penyunting di-snapshot (nama + peran) di samping `user_id` nullOnDelete - pola `mail_messages`. Diff dari `getChanges()` WAJIB disaring lagi untuk nilai yang sama secara angka: untuk kolom tanpa cast Eloquent membandingkan string, sehingga `-8.69` vs `-8.6900` terbaca berubah. Simpan tanpa perubahan = tanpa baris. Kode wilayah dicatat sebagai NAMA. Hak aksi di layar datang dari prop server `can` (`delete`, `warga`), bukan daftar peran di JSX (#101) | `app/Models/HydrantLog.php`, `app/Http/Controllers/Admin/HydrantController.php` |
...
| Empty state | Ad-hoc per halaman (teks fallback langsung di JSX); `CommandEmpty` hanya untuk combobox. **Pengecualian: keempat dashboard** memakai `AppEmpty` dari `Components/AppSection.jsx` sejak 2026-09-09 (#118) - keadaan kosong yang berbeda-beda rupa di empat layar terbaca seperti layar yang rusak, bukan seperti "memang belum ada isinya" (pelajaran TASK_45/#94). Jangan perluas ke halaman lain tanpa alasan yang sama | `resources/js/Components/ui/combobox.jsx`, `resources/js/Components/AppSection.jsx` |
...
| Kerangka dashboard | **Ada EMPAT dashboard** (`Pages/Dashboard.jsx` warga/relawan, `Pages/Admin/Dashboard.jsx` admin & pejabat, `Pages/Petugas/Dashboard.jsx`, `Pages/Opd/Dashboard.jsx`) dan keempatnya WAJIB lewat primitif bersama `Components/AppSection.jsx` (`AppGreeting`/`AppSection`/`AppList`/`AppListRow`/`AppEmpty`, FINDINGS #118). Alasannya bukan kerapian: bentuk lama - tiap layar membungkus daftarnya dengan kartu berbingkai sendiri - bukan kesalahan satu halaman melainkan **kebiasaan yang terulang empat kali**, dan memperbaikinya satu per satu melahirkan empat dialek (bentuk #71/#53). Bedanya mobile-vs-desktop dikerjakan lewat **breakpoint `md` saja**; jangan membuat pohon render kedua untuk ponsel. **`-mx-4` milik `AppList` terikat pada padding halaman `p-4` di `AppLayout`** - mengubah satu tanpa yang lain membuat daftar menonjol keluar layar atau menyisakan celah, tanpa galat maupun test merah. **DUA LARANGAN dari koreksi user 2026-09-09, dijaga `DashboardMobileShellTest`:** (a) tidak boleh ada margin negatif mendatar (`-mx-*`) - "jangan ada tampilan yang full kanan kiri harus tetap ada margin atau padding"; (b) tidak boleh ada `sticky`/`fixed` di kerangka dashboard - header `AppLayout` sudah `sticky`, dan pita lengket kedua membuat gulir terasa berlapis ("scrollnya menumpuk"). Keduanya MEMBALIK bentuk yang sempat dipasang hari itu juga, jadi jangan "perbaiki" kembali tanpa menanyakan user. Footer legal `AppLayout` boleh diringkas di ponsel tapi **tidak boleh disembunyikan**: sejak seksi nav "Bantuan & Legal" dihapus 2026-08-28 ia satu-satunya jalan ke halaman S&K/Privasi/Bantuan | `resources/js/Components/AppSection.jsx` |
