...
| Panel melayang di mobile | Panel yang dipicu dari bar bawah = popover melayang buatan tangan tepat di atas tombolnya (`absolute bottom-[72px]` + panah rotate-45, tutup lewat listener `mousedown` di luar). Ini keputusan user 2026-08-13 yang membalikkan TASK_21; `MobileMenuPanel.jsx` & `use-sheet-history.js` sudah DIHAPUS (ada di commit `2a1e2b6` bila perlu dipulihkan) — jangan hidupkan lagi pola drawer tanpa menanyakan user. `Components/ui/drawer.jsx` masih ada tapi kini tanpa pemakai. Padding `env(safe-area-inset-bottom)` pada bilah tetap wajib | `resources/js/Layouts/Partials/MobileBottomNav.jsx` |
