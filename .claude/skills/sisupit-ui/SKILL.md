...
| Layout & navigasi | `AppLayout` untuk semua halaman Admin & Front baru (bukan `AuthenticatedLayout`, itu legacy Breeze). **Menu ditulis SEKALI** di `Layouts/Partials/navItems.js` dan dipakai dua permukaan: `Sidebar.jsx` (desktop/tablet) dan `MobileBottomNav.jsx` (popover "Fasilitas" & "Menu" di bawah md). Pengecualian "dua daftar" 2026-08-13 sudah DICABUT 2026-08-19 (TASK_31) setelah sembilan menu terbukti hilang di ponsel — lihat #71. Menu baru: cukup tambah entri di `navItems.js`, ia otomatis mendarat di popover "Menu" | `resources/js/Layouts/Partials/navItems.js`, `resources/js/Layouts/Partials/MobileBottomNav.jsx` |
