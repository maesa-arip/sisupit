...
| Penanda "aktif" navigasi | Blok solid `bg-destructive` + `text-destructive-foreground` + `rounded-xl` (FINDINGS #72) di **sidebar** (`NavLink.jsx`). **Navigasi bawah mobile sejak 2026-09-01 TIDAK memakai bidang apa pun** (model minimalis, referensi user `docs/example/Menu 6.png`): aktif = `text-destructive font-semibold` saja — tanpa kotak, pil, kapsul, garis, atau titik. Ikon PADAT vs garis sengaja tidak dipakai sebagai pembeda (@tabler tak punya varian padat untuk semua ikon kita), begitu pula ketebalan garis (dicabut #72). Varian "garis tipis" pernah dicoba 2026-08-19 dan **ditolak user** — jangan dihidupkan lagi. **PENGECUALIAN (user 2026-08-20, PENGECUALIAN_ATURAN #2): baris di DALAM popover bottom-nav TIDAK memakai blok solid** — di sana aktif = tint 10% + teks sewarna jenis fasilitasnya (`bg-teal/10 text-teal` dst.), bentuk yang berjalan di production; blok solid terbaca seperti tombol darurat di dalam panel. Jangan "seragamkan" lagi tanpa menanyakan user. **Bentuk navigasi bawah (permintaan user 2026-09-01, referensi terukur `docs/example/Menu 6.png`):** MINIMALIS — bilah `h-16` selebar layar menempel `bottom-0`, `bg-card`, pemisahnya **box-shadow dua lapis** (`0 -1px 0 0 hsl(var(--border))` sebagai garis rambut yang terbaca di kedua mode + angkatan lembut untuk mode terang) — bukan `border-t`, sebab border menambah 1px ke tinggi kotak yang angkanya terikat di dua berkas lain; lima slot `grid-cols-5` berisi ikon **20px** (`h-5 w-5`, dinaikkan dari 16px atas permintaan user 2026-09-09) **kecuali slot "Lapor" yang 24px** (`h-6 w-6`, permintaan user hari yang sama - penonjolan slot darurat lewat UKURAN saja, bukan bidang/warna sendiri, sehingga #106 tak bisa kembali lewat pintu ini) + label **11px** ber-`font-normal` (aktif `font-medium`; keduanya diturunkan hari yang sama, "teksnya lebih slim ... hurufnya juga buat lebih kecil") berjarak `gap-2`. **Baris ikon dipatok 24px (`h-6`) untuk KELIMA slot** di `SlotContent` - tanpa itu ikon Lapor yang lebih tinggi mendorong labelnya turun sendirian dan satu label berdiri tidak sebaris dengan empat tetangganya; ukuran per slot dikirim lewat prop OPSIONAL `iconClassName` (menimpa lewat twMerge), satu jalur untuk kelima slot seperti `iconActive` - jangan bikin cabang khusus untuk satu slot, dan **tanpa bidang penanda aktif**. Dua bentuk lain sudah dicoba: **sepakat 1** (bilah 80px, penanda aktif kotak merah solid membungkus ikon + label) diarsipkan utuh di `docs/example/sepakat/` berikut cara memulihkannya — jangan tulis ulang dari ingatan; dan **kapsul melayang + tombol bulat** (referensi `menu 5.png`) yang dinilai user "kurang sesuai", ada di riwayat git. **DUA angka di luar berkasnya terikat pada tinggi bilah**: `AppLayout` (`pb-[calc(5rem+…)]`) dan tombol Kirim sticky `Front/Reports/Create.jsx` (`bottom-[calc(4rem+…)]` — rapat ke bilah, tanpa celah: dua bidang selebar layar dengan celah di antaranya memperlihatkan konten mengintip dan terbaca seperti tata letak yang meleset); mengubah satu tanpa yang lain membuat konten & tombol kirim laporan darurat tertutup bilah tanpa galat | `resources/js/Components/NavLink.jsx`, `resources/js/Layouts/Partials/MobileBottomNav.jsx` |
| Ikon di dalam navigasi | **Glyph monokrom yang mewarisi warna** (`currentColor`), tanpa pengecualian. Yang terlarang di bilah bukan MOTIF sebuah ikon melainkan **aset yang membawa warnanya sendiri**: ikon brand `/icon.png` dipakai di slot "Lapor" 2026-08-19 s/d 2026-09-01 lalu dilepas — berkas itu sendiri sebuah kotak merah utuh, jadi slot itu tampak aktif di SEMUA halaman dan tak pernah bisa ikut aturan bidang/glyph tetangganya (FINDINGS #106). Pelajarannya: **aturan warna hanya berlaku sejauh warna itu ditulis di KODE** — aset berwarna lolos dari setiap pemeriksaan yang membaca kelas. Sejak **2026-09-06** slot itu memakai petir brand lagi atas permintaan user, tapi sebagai GLYPH: `Components/BrandBoltIcon.jsx` — `<svg>` inline ber-`stroke="currentColor"` + `fill="none"`, kontrak props meniru @tabler (`size`/`stroke`/`className`), bentuk **garis bukan padat** (bidang terisi di antara glyph garis berbobot lebih berat tanpa alasan, ditolak di #106 putaran kedua). Ia karena itu ikut `text-destructive`/`text-muted-foreground` seperti empat tetangganya dan mustahil mengulangi #106. **Jangan pakai `IconBolt` @tabler** — petir @tabler sudah berarti jenis kejadian LISTRIK (`Admin/Dashboard.jsx`, `Admin/Agencies/Index.jsx`); nama komponennya sengaja dibedakan supaya keduanya tak tersambar autocomplete yang sama. Bilah memilih glyphnya sendiri untuk KELIMA slot (dipaku, seperti `IconDashboard`/`IconMapPin`/`IconHistory`/`IconMenu2`); yang TIDAK boleh dipaku adalah TUJUANNYA (aturan #71). Akibat yang disengaja: sidebar desktop tetap `IconFlame` untuk "Lapor Darurat!", jadi satu menu memakai dua ikon di dua permukaan — kalau itu tak dikehendaki, ubah `report.create` di `navItems.js`. `/icon.png` tetap hidup sebagai favicon & `ApplicationLogo`. **Sejak 2026-09-06 KELIMA slot MEMADAT saat aktif** (permintaan user): glyph garis saat diam, kembaran PADAT saat ia halaman yang sedang dibuka, lewat satu mekanisme `icon` + `iconActive` yang dipilih di `SlotContent`. Itu membalik aturan "padat vs garis bukan pembeda aktif" (#106 putaran ketujuh) — alasan aturan itu KEMAMPUAN dan MEMANG BENAR: `IconMenu2` tiga garis lurus terbuka & `IconHistory` busur terbuka, **garis tak punya bagian dalam** jadi `fill` di atasnya tak menghasilkan apa pun, dan tak ada kembaran padatnya. Karena itu **dua ikon DIGANTI**: `IconHistory` → `IconClock`, `IconMenu2` → `IconLayoutGrid`; Beranda & Fasilitas tak berubah rupa (`IconDashboardFilled`/`IconMapPinFilled` sudah ada). **Dicatat PENGECUALIAN_ATURAN #3.** Ini BUKAN #106 yang kembali: syaratnya "hanya saat aktif", dan "bidang terisi HANYA milik slot aktif" justru aturan yang lahir dari #106 putaran kedua. `iconActive` OPSIONAL & luruh ke glyph garis — itu yang menyelamatkan slot tamu "Masuk" (`IconLogin2`, tanpa kembaran padat), satu-satunya slot yang tak memadat. Bawaan `filled` `BrandBoltIcon` WAJIB tetap `false`; `BrandBoltIconFilled` diekspor tersendiri supaya petir brand dipanggil sama persis dengan pasangan @tabler — **jangan buat jalur khusus untuk satu slot**. Ketebalan garis tetap bukan pembeda (#72). Dijaga `MobileNavIconGlyphTest` (5 test) | `resources/js/Layouts/Partials/MobileBottomNav.jsx`, `resources/js/Components/BrandBoltIcon.jsx` |
...
| Panel melayang di mobile | Panel yang dipicu dari navigasi bawah = popover melayang buatan tangan tepat di atas tombolnya (`absolute bottom-[72px]` — angka ini terikat tinggi kapsul, lihat baris berikutnya; tutup lewat listener `mousedown` di luar), bertoken sama dengan `DropdownMenuContent` (`rounded-xl border bg-popover shadow-md`, TANPA panah — panah `rotate-45` dibuang di #72) dan baris `min-h-[48px]`. Ini keputusan user 2026-08-13 yang membalikkan TASK_21 dan **masih berlaku** (TASK_31 hanya mengganti sumber isinya, bukan bentuknya); `MobileMenuPanel.jsx` & `use-sheet-history.js` sudah DIHAPUS (ada di commit `2a1e2b6` bila perlu dipulihkan) — jangan hidupkan lagi pola drawer tanpa menanyakan user. `Components/ui/drawer.jsx` masih ada tapi kini tanpa pemakai. Padding `env(safe-area-inset-bottom)` pada bilah tetap wajib | `resources/js/Layouts/Partials/MobileBottomNav.jsx` |
| Nama peran akun | Kamus `ROLE_LABELS` + `roleLabel()`/`roleTone()` di `lib/utils.js` — **jangan** menyusun namanya lewat tangga `if` berantai. Bentuk lama di `Profile/Edit.jsx` hanya mengenali `relawan` dan `admin`/`petugas`, sehingga akun `opd`, `pejabat`, DAN `superadmin` sama-sama berbunyi "Anggota Masyarakat" di profilnya sendiri (FINDINGS #90) — cabang terakhir sebuah tangga bukan "tidak dikenal", melainkan sebuah KLAIM. Urutan daftarnya BERARTI (akun bisa berperan ganda; yang tampil yang paling menentukan wewenangnya), dan peran tak dikenal berbunyi "Peran belum ditetapkan". Menambah peran = satu baris di kamus itu. **Peran warga bernama `masyarakat` berlabel "Anggota Masyarakat" sampai 2026-09-02**; sejak itu keduanya berbunyi `warga`/"Warga" (permintaan user). Dijaga `RoleLabelParityTest` | `resources/js/lib/utils.js`, `resources/js/Pages/Profile/Edit.jsx` |
| Memilih tanggal & jam | **Selalu `Components/DatePicker.jsx` / `Components/TimePicker.jsx`, JANGAN `<input type="time">` atau `type="date"`.** Input tanggal/jam bawaan tidak digambar halaman - ia menyerahkan pemilihannya ke **dialog NATIVE** milik WebView, dan di APK Android mengetuk pemilih jam MENUTUP APLIKASI di tengah pengisian Berita Acara (FINDINGS #108). Gejalanya **tidak pernah muncul di browser desktop**, tempat perubahan frontend biasanya diperiksa - jadi ia lolos review dan baru ketahuan dari laporan pengguna di lapangan. Kedua komponen itu murni JavaScript, berpasangan bentuk (Popover + trigger `Button variant="outline"` `h-10`), dan berbicara STRING (`'YYYY-MM-DD'` / `'HH:mm'`) supaya cocok dengan pola form Inertia. Kalau butuh granularitas lain, ubah isinya - jangan kembali ke input native. Dijaga `FormControlNativeDialogTest` | `resources/js/Components/{DatePicker,TimePicker}.jsx` |
