...
| Ikon di dalam navigasi | **Glyph monokrom yang mewarisi warna** (`currentColor`), tanpa pengecualian. Bilah bawah sempat memakai ikon brand `/icon.png` di slot "Lapor" (2026-08-19 s/d 2026-09-01); **sudah dilepas** atas permintaan user — berkas itu sendiri sebuah kotak merah utuh, jadi satu slot selalu berbahasa visual sendiri dan tak pernah bisa ikut aturan bidang/glyph tetangganya (ia bahkan tampak aktif di semua halaman, FINDINGS #106). Slot itu kini memakai `IconFlame` yang diambil dari item `report.create` milik `navItems.js` — jangan dipaku, dan jangan pakai `IconBolt` (petir sudah berarti jenis kejadian LISTRIK di `Admin/Dashboard.jsx`). `/icon.png` tetap hidup sebagai favicon & `ApplicationLogo` | `resources/js/Layouts/Partials/MobileBottomNav.jsx` (`SlotContent`, `slotClass`) |
