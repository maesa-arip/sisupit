...
| Layout & navigasi | `AppLayout` untuk semua halaman Admin & Front baru (bukan `AuthenticatedLayout`, itu legacy Breeze). Menu role-aware untuk desktop/tablet ditulis di `Layouts/Partials/navItems.js`, dirender `Sidebar.jsx`. **Perkecualian sadar (2026-08-13, permintaan user):** `MobileBottomNav.jsx` dikembalikan ke bentuk pra-TASK_20 dan memelihara daftarnya SENDIRI — menu baru yang harus tampil di ponsel wajib ditulis di dua tempat. Lihat catatan pembalikan #53/#54 di FINDINGS_LOG | `resources/js/Layouts/Partials/navItems.js`, `resources/js/Layouts/Partials/MobileBottomNav.jsx` |
| Panel melayang di mobile | Panel yang dipicu dari bar bawah = popover melayang buatan tangan tepat di atas tombolnya (`absolute bottom-[72px]` + panah rotate-45, tutup lewat listener `mousedown` di luar). Ini keputusan user 2026-08-13 yang membalikkan TASK_21; drawer vaul (`MobileMenuPanel.jsx`) & `useSheetHistory` masih ada di repo tapi TIDAK dipakai — jangan pakai lagi tanpa menanyakan user. Padding `env(safe-area-inset-bottom)` pada bilah tetap wajib | `resources/js/Layouts/Partials/MobileBottomNav.jsx` |
