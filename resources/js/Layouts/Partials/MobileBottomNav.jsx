 ...
 * (Material 3: 3–5 destinasi): Beranda, Fasilitas, Lapor, Riwayat, Menu.
 *
 * Slot "Menu" membuka Sheet berisi komponen <Sidebar/> yang SAMA dengan desktop —
 * bukan salinan daftar menu tersendiri. Sebelumnya bottom-nav memelihara daftarnya
 * sendiri, akibatnya seksi "Bantuan & Legal" tak pernah tampil di mobile dan menu
 * admin menumpuk sampai 13 item di dalam popover 208px yang harus di-scroll.
 */
export default function MobileBottomNav({ auth }) {
	...
	// Tutup Sheet begitu Inertia berpindah halaman. NavLink di dalam <Sidebar/> tidak tahu
	// apa-apa soal Sheet, jadi penutupannya dipicu dari perubahan url — bukan onClick tiap item.
	useEffect(() => {
		setShowMenu(false);
	...
	return (
		<>
			...
			<div className="fixed bottom-0 left-0 z-50 h-16 w-full border-t border-border bg-card md:hidden">
				<div className="mx-auto grid h-full max-w-md grid-cols-5 px-1">
					...
					<button
						...
						className={cn(
							'group z-50 flex h-full w-full flex-col items-center justify-center gap-1 outline-none transition-colors',
							showMenu ? 'text-destructive' : 'text-muted-foreground hover:text-destructive',
						)}
					>
						<div
							className={cn(
								'flex items-center justify-center rounded-full px-4 py-1.5 transition-colors group-focus-visible:ring-2 group-focus-visible:ring-destructive',
								showMenu ? 'bg-destructive/10' : 'bg-transparent group-hover:bg-destructive/10',
							)}
						>
							<IconMenu2 className="h-7 w-7" stroke={showMenu ? 2 : 1.5} />
						</div>
			...
			{/* Panel menu penuh: satu-satunya wadah yang muat menampung seluruh seksi
			    (Administrasi superadmin sendirian sudah 13 item). */}
			<Sheet open={showMenu} onOpenChange={setShowMenu}>
				<SheetContent side="right" className="flex w-[86vw] max-w-sm flex-col gap-0 p-0">
					<SheetHeader className="shrink-0 border-b border-border px-5 py-4 text-left">
						<SheetTitle className="text-base font-bold">
							{auth?.name || auth?.user?.name ? 'Menu' : 'Menu & Informasi'}
						</SheetTitle>
					</SheetHeader>
