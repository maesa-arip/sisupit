...
const UserLeafletMap = ({ markers = [] }) => {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    // Ikon Lokasi User (Bentuk Pin Map Merah)
                ...
                (err) => console.warn('Gagal mendapatkan lokasi GPS:', err.message)
            );
        }
    ...
    // ==========================================
    // EFFECT 2: RENDER MARKER (Otomatis bedakan Pompa & Pos)
    // ==========================================
    useEffect(() => {
        ...
        // Fungsi Pembuat Ikon Pintar (Cek Kategori)
        const createCustomIcon = (status, category) => {
            ...
            if (isPosPemadam) {
                ...
                // Tema Pompa (Biru untuk Aktif, Oranye untuk Perbaikan)
        ...
        if (markers && markers.length > 0) {
            markers.forEach((marker) => {
                ...
                if (!isNaN(lat) && !isNaN(lng)) {
                    // PERBAIKAN: Lempar category ke fungsi icon
                    const customIcon = createCustomIcon(marker.status, marker.category);
                    
                    // Logika text popup agar menyesuaikan jenis asetnya
