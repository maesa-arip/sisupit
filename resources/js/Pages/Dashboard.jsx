...
export default function Dashboard(props) {
	...
	// BADGE STATUS (Disesuaikan dengan palet flat ReportCard)
	const StatusBadge = ({ status }) => {
		const variants = {
			...
			resolved: {
				className:
					'bg-gray-100 text-gray-600 border-[#e5e5e5] dark:bg-[#1a1a1a] dark:border-[#333] dark:text-gray-400',
				label: 'Selesai',
			},
			TERLAPOR: {
				className:
					'bg-red-50 text-[#b42826] border-red-200 dark:bg-red-900/20 dark:border-red-900/50 dark:text-red-400',
				label: 'Butuh Bantuan',
			},
		};
	...
	// --- KOMPONEN BLOK: RIWAYAT LAPORAN SAYA ---
	...
	const RenderRadarFeed = () => (
		<div className="space-y-4">
			<div className="flex flex-col gap-4">
				...
				{/* TABS (Flat Design Monokrom & Merah) */}
				{isRelawan && (
	...
	return (
		<div className="flex flex-col w-full pb-32 mx-auto space-y-6 max-w-7xl">
			...
			{/* BANNER AKSI (Flat, Monokrom, & Kompak) */}
				...
				{isRelawan ? (
					...
					<Card className="overflow-hidden rounded-xl border border-[#e5e5e5] bg-white shadow-none transition-colors dark:border-[#262626] dark:bg-[#151515]">
						<CardContent className="flex flex-col justify-between gap-3 p-4 sm:flex-row sm:items-center">
							...
							{/* 👇 FIX: TOMBOL DAFTAR RELAWAN EKSTRA KOMPAK (h-8) 👇 */}
							<Button className="h-8 w-full shrink-0 rounded-md border border-transparent bg-gray-900 px-4 text-[10px] font-bold uppercase tracking-wider text-white shadow-none transition-colors hover:bg-black dark:bg-white dark:text-black dark:hover:bg-gray-200 sm:w-auto">
								Daftar Relawan
							</Button>
						</CardContent>
					</Card>
			...
			{/* LAYOUTING CERDAS BERDASARKAN ROLE */}
			{isRelawan ? (
				<>
					<RenderRadarFeed />
