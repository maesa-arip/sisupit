...
export default function Edit({ pump, pump_province, tenant_location, provinces, cities, districts, admin_level, admin_region_names }) {
	...
	const { data, setData, put, processing, errors } = useForm({
		...
		capacity_lpm: pump.capacity_lpm ?? '',
		description: pump.description || '',
		lat: parseFloat(pump.lat) || defaultLat,
		lng: parseFloat(pump.lng) || defaultLng,
		province_code: pump_province || admin_level?.province_code || '',
		city_code: pump.city_code || admin_level?.city_code || '',
		district_code: pump.district_code || admin_level?.district_code || '',
		village_code: pump.village_code || admin_level?.village_code || '',
	});
	...
	const onHandleSubmit = (e) => {
		e.preventDefault();
		put(route('admin.pumps.update', pump.id), {
			preserveScroll: true,
			preserveState: true,
			onSuccess: (page) => {
	...
	return (
		<div className="flex flex-col w-full h-full space-y-6">
			<Head title={`Modifikasi Aset: ${pump.name}`} />

			<div className="flex flex-col items-start justify-between mb-2 gap-y-4 lg:flex-row lg:items-center">
				<HeaderTitle title="Modifikasi Data Pompa" subtitle="Perbarui teknis, yurisdiksi, atau re-posisi titik aset." icon={IconEngine} />
							...
							<form className="space-y-5" onSubmit={onHandleSubmit}>
								...
								<div className="grid grid-cols-2 gap-4">
									<div className="grid gap-1.5">
										...
										<Select defaultValue={data.type} onValueChange={(value) => setData('type', value)}><SelectTrigger className="focus-visible:ring-teal-500 dark:focus-visible:ring-teal"><SelectValue placeholder="Pilih Jenis" /></SelectTrigger><SelectContent><SelectItem value="Statis (Hydrant)">Statis (Hydrant)</SelectItem><SelectItem value="Portable (Mobil)">Portable (Mobil)</SelectItem><SelectItem value="Mesin Diesel">Mesin Diesel</SelectItem></SelectContent></Select>
										{errors.type && <InputError message={errors.type} />}
									</div>
									<div className="grid gap-1.5">
										<Label>Status Operasional</Label>
										<Select defaultValue={data.status} onValueChange={(value) => setData('status', value)}><SelectTrigger className="focus-visible:ring-teal-500 dark:focus-visible:ring-teal"><SelectValue placeholder="Pilih Status" /></SelectTrigger><SelectContent><SelectItem value="Aktif">Siap Pakai</SelectItem><SelectItem value="Perbaikan">Perbaikan</SelectItem></SelectContent></Select>
								...
								<div className="grid gap-1.5">
									...
									<Input type="number" name="capacity_lpm" id="capacity_lpm" value={data.capacity_lpm} onChange={onHandleChange} className="focus-visible:ring-teal-500 dark:focus-visible:ring-teal" />
								...
								<div className="flex justify-end gap-2 pt-2">
									<Button type="button" variant="secondary" asChild><Link href={route('admin.pumps.index')}>Batal</Link></Button>
...
Edit.layout = (page) => <AppLayout children={page} title="Modifikasi Pompa" />;
