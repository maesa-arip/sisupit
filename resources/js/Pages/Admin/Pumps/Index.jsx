...
export default function Index({ pumps, filters, tenant_location }) {
		...
		if (pumps.data && pumps.data.length > 0) {
			pumps.data.forEach((pump) => {
				...
				if (!isNaN(lat) && !isNaN(lng)) {
					const iconColor = pump.status === 'Aktif' ? 'text-teal-600 dark:text-teal' : 'text-amber-500 dark:text-warning';
	...
	const confirmDelete = () => {
		if (pumpToDelete)
			router.delete(route('admin.pumps.destroy', pumpToDelete), {
				preserveScroll: true,
				...
				<div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
					<div className="w-full max-w-md p-6 border shadow-xl rounded-2xl bg-background">
						...
						<p className="mt-2 text-sm text-muted-foreground">
							Menghapus pompa ini akan menghilangkan koordinatnya dari peta operasional secara permanen.
						</p>
			...
			<div className="flex flex-col items-start justify-between gap-y-4 sm:flex-row sm:items-center">
				...
				<Button size="sm" className="text-white bg-teal-600 dark:bg-teal border-none shadow-none hover:bg-teal-700 dark:hover:bg-teal/90" asChild>
					<Link href={route('admin.pumps.create')}>
						<IconPlus className="mr-1.5 h-4 w-4" /> Tambah Pompa
					</Link>
				</Button>
			</div>
							...
							<>
								{pumps.data.map((pump) => (
									<Card
										...
										className={`cursor-pointer transition-colors ${activePumpId === pump.id ? 'border-teal-500 dark:border-teal bg-teal-50/50 dark:bg-teal/5' : 'hover:border-teal-300 dark:hover:border-teal/50'}`}
									>
										<CardContent className="flex flex-col gap-3 p-3 sm:p-4">
											<div className="flex flex-row items-center gap-3">
												<div
													className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${pump.status === 'Aktif' ? 'bg-teal-100 dark:bg-teal/10 text-teal-600 dark:text-teal' : 'bg-amber-100 dark:bg-warning/10 text-amber-600 dark:text-warning'}`}
												>
													{pump.status === 'Aktif' ? (
...
Index.layout = (page) => <AppLayout children={page} title="Manajemen Pompa" />;
