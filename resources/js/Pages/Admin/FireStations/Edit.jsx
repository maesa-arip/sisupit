...
export default function Edit({ station, station_province, tenant_location, provinces, cities, districts, admin_level, admin_region_names }) {
	...
	const { data, setData, put, processing, errors } = useForm({
		...
		phone: station.phone || '',
		vehicle_count: station.vehicle_count ?? 1,
		lat: parseFloat(station.lat) || defaultLat,
		lng: parseFloat(station.lng) || defaultLng,
		province_code: station_province || admin_level?.province_code || '',
		city_code: station.city_code || admin_level?.city_code || '',
		district_code: station.district_code || admin_level?.district_code || '',
		village_code: station.village_code || admin_level?.village_code || '',
	});
	...
	const onHandleSubmit = (e) => {
		e.preventDefault();
		put(route('admin.fire-stations.update', station.id), {
			preserveScroll: true,
			preserveState: true,
			onSuccess: (page) => {
...
Edit.layout = (page) => <AppLayout children={page} title="Modifikasi Pos Pemadam" />;
